...
function expKey(maxHeat, minHeat) {
    ...
    var keyText = keyTextContainer.selectAll('text')
        ...
        .attr('fill', 'black')
        .each(function (d) {
            d.titleWidth = this.getBBox().width;
        })
        .attr('y', recSize * .75);
    ...
    var key = mapKey.selectAll('rect')
        ...
        .append('rect')
        .attr('x', function (d, i) {
            return recSize * (i + .5) + pushForward;
        })
        .attr('y', recSize * .25)
...
function expSub(heatMap, maxHeat, minHeat) {
    ...
    var fontRatio = graphContainer.append('g')
        ...
        .each(function (d) {
            d.charHeight = this.getBBox().height;
        })
        .each(function (d) {
            d.charWidth = this.getBBox().width;
        });

    charHeight = d3.max(ratioChar, function (d) {
        return d.charHeight;
    });
    charWidth = d3.max(ratioChar, function (d) {
        return d.charWidth;
    });

    ratio = charWidth / charHeight;
    t = 1000;
    var n = d3.min(heatMap, function (d, i) {
        if (d.name.length <= maxLength || maxLength == 0) {
            return t / ((i + .5) + (d.name.length * ratio * .707));
        }
        else {
            return t / ((i + .5) + (maxLength * ratio * .707));
        }
    });
...
////////////////////
    ...
    var graphBackground = graphContainer.append('g')
        ...
        .attr('xlink:href', function (d) {
            if (d.node) {
                return '/biomaterial/' + d.name;
            }
            return null;
        })
        .append('rect')
        .each(function (d) {
            d.intensityLabel = d.intensity;
        })
        .each(function (d) {
            d.intensityLabel = Number(d.intensity) + Number(minNorm);
        });

    var graphBackgroundConstruct = graphBackground
        .attr('transform', function (d, i) {
            return 'translate(' + i * colWidth + ',' + 0 + ')';
        })
        .attr('height', graphHeight)
        .attr('width', colWidth)
            ...
            div.html(function () {
                ...
                .style('border-color', function () {
                    ...
                    //	}
                        ...
                        return d3.rgb(0, (d.intensity * hF), 0);
                    }
                })
        ...
        //.on("click", function(d) {window.location = window.location.href +
        // "/biomaterial/"+d.name;})
...
///////////////////
    ...
    var graphLables = graphContainer.append('g')
        ...
        .append('text')
        .attr('x', function (d, i) {
            return (i * colWidth + (.5 * colWidth) - (.25 * colWidth));
        })
        ...
        .each(function (d) {
            d.titleHeight = 0.707 * this.getBBox().width;
        })
        .each(function (d, i) {
            d.titleend = (colWidth * i + (.5 * colWidth) - (.25 * colWidth)) + (0.707 * this.getBBox().width);
        });


    var maxTitleHeight = d3.max(heatMap, function (d) {
        return d.titleHeight;
    });
    var divWidth = d3.max(heatMap, function (d) {
        return d.titleend;
    });
